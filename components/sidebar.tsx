...
const affiliate = [...privileged, "supervisor", "responsable", "afiliado", "promotor"];
const financial = [...privileged, "supervisor", "responsable", "vendedor", "afiliado", "promotor"];

const links: NavLink[] = [
  { href: "/", label: "Dashboard", icon: LayoutDashboard },
  { href: "/finanzas", label: "Finanzas", icon: Banknote, roles: financial },
  ...
  { href: "/afiliados", label: "Afiliados", icon: Handshake, roles: affiliate },
  { href: "/referidos", label: "Referidos", icon: Share2, roles: affiliate },
  ...
  { href: "/ventas", label: "Ventas", icon: ShoppingCart, roles: financial },
  { href: "/pagos", label: "Registrar cobros", icon: Receipt, roles: [...privileged, "supervisor", "responsable", "vendedor"] },
  ...
  { href: "/reportes", label: "Reportes", icon: BarChart3, roles: financial },
...
export function Sidebar({ role }: SidebarProps) {
  ...
  const visibleLinks = links.filter((link) => {
    ...
    return Boolean(normalizedRole) && link.roles.includes(normalizedRole);
  });
    ...
    <>
      ...
      >
        <div className="mb-5 flex items-center justify-between">
          ...
          <button type="button" onClick={() => setIsMobileOpen(false)} aria-label="Cerrar menú" className="grid h-10 w-10 place-items-center rounded-2xl border border-white/10 bg-white/[0.06] text-white">
            <X size={20} />
          </button>
        </div>
